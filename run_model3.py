...
print("=" * 60)
print("MODEL 3: +GATING (Time2Vec + Market Gating)")
print("=" * 60)
...
print(f"Input dim: {in_d}, Market dim: {mkt_d}")
...
for fold_data in splits_gen:
    ...
    tr_m = fold_data["train"]["market_features"].values

    vl_f = fold_data["val"]["features"].values
    vl_t = fold_data["val"]["targets"]["target_h1"].values
    vl_m = fold_data["val"]["market_features"].values

    te_f = fold_data["test"]["features"].values
    te_t = fold_data["test"]["targets"]["target_h1"].values
    te_m = fold_data["test"]["market_features"].values

    train_ds = TimeSeriesDataset(tr_f, tr_t, tr_m, 60, include_time)
    val_ds = TimeSeriesDataset(vl_f, vl_t, vl_m, 60, include_time)
    test_ds = TimeSeriesDataset(te_f, te_t, te_m, 60, include_time)
...
with open(os.path.join(RESULT_DIR, "model3_gating.json"), "w") as f:
    json.dump(model_results, f, indent=2)

np.save(os.path.join(RESULT_DIR, "model3_preds.npy"), all_preds)
np.save(os.path.join(RESULT_DIR, "model3_targs.npy"), all_targs)

print(f"\n✓ Results saved to results/model3_gating.json")
