#!/usr/bin/env python
"""Run Transformer with Sharpe Loss - for comparison with MSE."""
...
print("=" * 60)
print("MODEL 6: TRANSFORMER + SHARPE LOSS")
print("=" * 60)
...
for fold_data in splits_gen:
    ...
    # Use Sharpe loss instead of MSE
    train_model(
        model,
        train_loader,
        val_loader,
...
print(f"\n[4/4] FINAL RESULTS - TRANSFORMER+SHARPE:")
print("-" * 40)
...
# Save
model_results = {
    "model": "Transformer_SharpeLoss",
    "metrics": {k: float(v) for k, v in final_metrics.items()},
    "n_folds": fold_num,
    "fold_results": [{k: float(v) for k, v in f.items()} for f in fold_results],
}

with open(os.path.join(RESULT_DIR, "model6_transformer_sharpe.json"), "w") as f:
    json.dump(model_results, f, indent=2)

np.save(os.path.join(RESULT_DIR, "model6_preds.npy"), all_preds)
np.save(os.path.join(RESULT_DIR, "model6_targs.npy"), all_targs)

print(f"\n✓ Results saved to results/model6_transformer_sharpe.json")
